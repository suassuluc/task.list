...
@section('content')
    <form method="POST" action="{{ isset($task) ? route('tasks.update', ['task' => $task->id]) : route('tasks.store') }}">
        ...
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('tasks.index') }}" class="btn-del">Cancel</a>
        </div>
    </form>
@endsection


{{-- {{old()}} metodo old e utilizado para resgatar informação antiga no sistema, por exemplo uma pessoa fez uma atribuição indevida e recarrega a pagina
esse metodo mantem os dados sem apagalos --}}
