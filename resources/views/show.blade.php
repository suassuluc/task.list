...
@section('content')
 {{-- @dd($task) --}}
    ...
    Description:<p class="mb-4 text-slate-700">{{ $task->description }}</p>
    ...
    <div class="flex gap-2">
        <a href="{{ route('tasks.edit', ['task' => $task]) }}" class="btn-edt">Edit
        </a>
        <form method="POST" action="{{ route('tasks.toggle-complete', ['task' => $task]) }}">
            @csrf
            @method('PUT')
            <button type="submit" class="btn-cmp">
        ...
        <form action="{{ route('tasks.destroy', ['task' => $task]) }}" method="POST">
            @csrf
            @method('DELETE')
            <button type="submit" class="btn-del">Delete
            </button>
        </form>
    </div>
